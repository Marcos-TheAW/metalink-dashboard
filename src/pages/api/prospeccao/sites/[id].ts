...
import { atualizarSiteProspectado } from '../../../../lib/db';
import {
  CANAIS_PROSPECCAO,
  OPCOES_SIM_NAO,
  OPCOES_TRI_ESTADO,
...
export const POST: APIRoute = async ({ request, params, locals, redirect }) => {
  ...
  const erros: string[] = [];
  if (!Number.isFinite(id)) erros.push('Site inválido.');
  ...
  if (erros.length > 0) {
    return redirect(`/prospeccao/sites/${id}?erro=${encodeURIComponent(erros.join(' '))}`);
  }
