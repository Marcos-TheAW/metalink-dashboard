...
import { CABECALHO_MODELO_PEDIDOS } from '../../../../lib/importacao';
...
export const GET: APIRoute = async ({ locals }) => {
  ...
  const planilha = XLSX.utils.aoa_to_sheet([CABECALHO_MODELO_PEDIDOS]);
  // Larguras de coluna aproximadas para o cabeçalho ficar legível ao abrir no Excel/Sheets.
  planilha['!cols'] = CABECALHO_MODELO_PEDIDOS.map((titulo) => ({ wch: Math.max(titulo.length, 12) }));
  ...
  return new Response(arquivo, {
    headers: {
      ...
      'Content-Disposition': 'attachment; filename="modelo-pedidos.xlsx"'
    }
  });
};
